...
dg.prcomp <- function(data, check.plot = FALSE, UserMenus = NULL,
  ...
  Z <- DynamicGraph(Vertices = Vertices, ExtraVertices = ExtraVertices,
                    ExtraEdges = ExtraEdges, title = "Pre-rotated",
  ...
  U <- DynamicGraph(Vertices = Vertices, ExtraVertices = ExtraVertices,
                    ExtraEdges = ExtraEdges, object = Object, w = 1, 
                    UserMenus = UserMenus, title = "Master", returnLink = TRUE)

  W <- DynamicGraph(Vertices = Vertices, ExtraVertices = ExtraVertices,
                    ExtraEdges = ExtraEdges, transformation = t(Loadings), 
                    title = "With transformation", object = Object, w = 1, 
                    UserMenus = UserMenus, addModel = TRUE, frameModels = U)
