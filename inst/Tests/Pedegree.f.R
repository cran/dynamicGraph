...
# Test with block recursive model, outer blocks vertically :

source("startup.0.R")

Block.tree <- 
 list(f = 0.08, F = 0.15, horizontal = FALSE, label = "Pedegree.f",
      ...
      list(f = 0.1, F = 0.2, horizontal = FALSE,
           ...
           list(f = 0.15, F = 0.25, horizontal = FALSE,
                ...
                common.children = 
                list(Vertices = c("Marriage"), f = 0.4, F = 0.5,
                     Son = 
                     list(Vertices = c("Son"), f = 0.50,
                          P.G.Son = list(Vertices = c("P.G.Son"), f = 0.33),
                          P.G.Dat = list(Vertices = c("P.G.Dat"), f = 0.33)),
                     Dat = 
                     list(Vertices = c("Dat"), f = 0.50,
...
Z <- DynamicGraph(V.Names, V.Types, From, To, 
                  ...
                  UserMenus = Menus, title = "Pedegree.f")
